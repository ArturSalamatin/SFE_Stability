...
% Tau0_vals = [0.03, 0.2];
Tau0_vals = linspace((1+1e-4)*(A0_vals^2)/2, 0.5*(1-1e-5), 500);
...
for i = 1:numel(R_vals)
    ...
    for j = 1:numel(Alpha_vals)
        ...
        %% run calculations
%         Sigma(j,:,:) = ...
%             calc_sigma_full_3(solver, N, alpha, tau0, a0, R, B_vals, pens);
        
        sigma = reshape(Sigma(j,:,:), size(Sigma, [2,3]));
        
        figure(num)
        plot(Tau0_vals, sigma(1,:), '-',  'Color', cols{j}, 'LineWidth', 1)
        plot(Tau0_vals, sigma(2,:), '--', 'Color', cols{j}, 'LineWidth', 1)
        plot(Tau0_vals, sigma(3,:), '-.', 'Color', cols{j}, 'LineWidth', 1)
        drawnow;
    end
    toc
    save(['full_data/fixed_B/R_', num2str(R*100), '.mat'], ...
        'R_vals', 'Sigma')
