...
R_vals = [0.1, 0.3, 1, 3];
h_vals = linspace(0, 10, 20);

A0_vals = 0.2;
Alpha_vals = [0, 0.1, 0.3, 0.5, 0.7, 0.9];
Tau0_vals = [0.03];
...
for i = 1:numel(R_vals)
    ...
    for j = 1:numel(Alpha_vals)
        ...
        %% run calculations
%         Sigma(j,:,:) = ...
%             calc_sigma_full(solver, N, alpha, tau0, a0, R_vals, h_vals, pens);
%         sigma = reshape(Sigma(j,:,h_ids), numel(R_vals), numel(h_vals(h_ids)));
%         
%         plot(R_vals, sigma, 'Color', cols{j}, 'LineWidth', 1)
    end
    toc
